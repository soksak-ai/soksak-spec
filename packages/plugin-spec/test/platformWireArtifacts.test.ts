...
import { parsePlatformReleaseManifest } from "../src/platformRelease.js";
...
describe("portable platform wire artifacts", () => {
  it("publishes strict draft-2020-12 schemas for every public JSON boundary", () => {
    ...
    const platformRelease = json(join(SCHEMAS, "platform-release.schema.json"));
    ...
    for (const schema of [release, platformRelease, conformance, registry, registryPublicKey]) {
      expect(schema.$schema).toBe("https://json-schema.org/draft/2020-12/schema");
      expect(schema.additionalProperties).toBe(false);
    }
    expect(release.properties.spec.const).toBe("soksak-spec-release@0.0.1");
    expect(platformRelease.properties.spec.const).toBe("soksak-spec-platform-release@0.0.1");
    ...
    expect(platformRelease.$id).toBe("urn:soksak:spec:platform-release:0.0.1");
    ...
    for (const schema of [release, platformRelease, conformance, registry]) {
  ...
  it("compiles every schema and accepts the same valid cross-language corpus", () => {
    ...
    const validators = {
      ...
      platformRelease: ajv.compile(json(join(SCHEMAS, "platform-release.schema.json"))),

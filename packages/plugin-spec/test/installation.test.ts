...
describe("environment ownership", () => {
  it("accepts materialized components without plugin role bindings", () => {
    expect(parseEnvironmentDocument({ ...empty(), plugins: { demo: { version: "0.0.1", path: "/installed/demo", source: "registry", registry: "official", enabled: true } } })).toMatchObject({ ok: true });
    expect(parseEnvironmentDocument({ ...empty(), plugins: { demo: { version: "0.0.1", path: "/installed/demo", source: "registry", registry: "official", enabled: true, sidecars: { terminal: "terminal-provider" } } } }).ok).toBe(false);
  });

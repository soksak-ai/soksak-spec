...
import { ARTIFACT_FORMATS, COMPONENT_ID_RE, GIT_COMMIT_RE, RUST_SIDECAR_TARGETS, SHA256_RE, STRICT_SEMVER_RE, type ArtifactFormat, type ReleaseKind, type RustSidecarTarget } from "./release-primitives.js";
...
export interface IntegrityReference { url: string; size: number; sha256: string }
...
const REPOSITORY_RE = new RegExp("^https://github[.]com/[A-Za-z0-9](?:[A-Za-z0-9-]{0,37}[A-Za-z0-9])?/[A-Za-z0-9][A-Za-z0-9._-]{0,99}$");
...
function releaseAsset(value: unknown, repository: string, version: string): value is string {
...
function parseIntegrity(raw: unknown, label: string, repository: string, version: string, errors: string[]): IntegrityReference | null {
  ...
  const value = strict(raw, ["sha256", "size", "url"], ["sha256", "size", "url"], label, errors);
  if (!value) return null;
  if (!releaseAsset(value.url, repository, version)) errors.push(`${label}.url: exact v${version} release asset required`);
  ...
  return errors.length === before ? { url: value.url as string, size: value.size as number, sha256: value.sha256 as string } : null;
}
...
function parseArtifacts(raw: unknown, kind: ReleaseKind, repository: string, version: string, errors: string[]): ReleaseArtifact[] {
  ...
  raw.forEach((item, index) => {
    ...
    const before = errors.length;
    const value = strict(item, ["format", "manifest", "sha256", "size", "target", "url"], ["format", "manifest", "sha256", "size", "target", "url"], label, errors);
    if (!value) return;
    if (!releaseAsset(value.url, repository, version)) errors.push(`${label}.url: exact v${version} release asset required`);
    if (!Number.isSafeInteger(value.size) || (value.size as number) <= 0) errors.push(`${label}.size: positive safe integer required`);
    if (typeof value.sha256 !== "string" || !SHA256_RE.test(value.sha256)) errors.push(`${label}.sha256: exact SHA-256 required`);
    ...
    if (errors.length === before) output.push({ url: value.url as string, size: value.size as number, sha256: value.sha256 as string, target: value.target as ReleaseArtifact["target"], format: value.format as ArtifactFormat, manifest: value.manifest as ReleaseArtifact["manifest"] });
  });
...
export function parseReleaseManifest(raw: unknown): PlatformParseResult<ReleaseDocument> {
  ...
  if (!sourceValue || typeof sourceValue.repository !== "string" || !REPOSITORY_RE.test(sourceValue.repository)) errors.push("release.source.repository: canonical GitHub repository required");
  ...
  const manifest = parseIntegrity(value.manifest, "release.manifest", source.repository, version, errors);
  if (manifest && !manifest.url.endsWith(`/${expectedManifest(kind)}`)) errors.push(`release.manifest.url: ${expectedManifest(kind)} required`);
  const artifacts = parseArtifacts(value.artifacts, kind, source.repository, version, errors);
  ...
  else value.evidence.forEach((item, index) => { const parsed = parseIntegrity(item, `release.evidence[${index}]`, source.repository, version, errors); if (parsed) evidence.push(parsed); });
  sortedUnique(evidence.map((item) => item.url), "release.evidence", errors);

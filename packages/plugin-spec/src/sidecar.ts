...
export function parseSidecarManifest(raw: unknown): { ok: true; value: SidecarManifest } | { ok: false; errors: string[] } {
  ...
  checkKnownKeys(raw, ["id", "interface", "process", "spec", "version"], "sidecar", errors);
  for (const key of ["id", "interface", "process", "spec", "version"]) {
    if (!(key in raw)) errors.push(`sidecar.${key}: required`);
  }
  if (raw.spec !== "soksak-spec-sidecar@0.0.1") errors.push("sidecar.spec: soksak-spec-sidecar@0.0.1 required");
  ...
  return { ok: true, value: { spec: "soksak-spec-sidecar@0.0.1", id: raw.id, version: "0.0.1", interface: interfaceRef, process: raw.process as string } };
}

...
test("spec release projects the derived owner identity", () => {
  ...
  const release = buildPlatformRelease({
    commit,
    archiveName: "soksak-ai-plugin-spec-0.0.28.tgz",
    archiveDigest: digest,
    archiveSize: 81840,
    identity: specReleaseIdentity(workspace, pluginSpec),
    manifestBytes: Buffer.from('{}\n'),
  });
  assert.deepEqual({ kind: release.kind, id: release.id, version: release.version }, { kind: "spec", id: "soksak-spec", version: "0.0.28" });

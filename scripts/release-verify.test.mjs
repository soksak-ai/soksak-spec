...
test("spec release projects the derived owner identity", () => {
  ...
  const release = buildPlatformRelease({
    commit,
    archiveName: `soksak-soksak-spec-${workspace.version}.tgz`,
    archiveDigest: digest,
    archiveSize: 81840,
    identity: specReleaseIdentity(workspace, pluginSpec),
    manifestBytes: Buffer.from('{}\n'),
  });
  assert.deepEqual({ kind: release.kind, id: release.id, version: release.version }, { kind: "spec", id: "soksak-spec", version: workspace.version });
...
test("spec release names every file by bare name; the location is derived by the reader", () => {
  ...
  assert.equal(release.artifacts[0].file, `soksak-soksak-spec-${workspace.version}.tgz`);
